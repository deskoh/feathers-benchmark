...
async function connectClient(url, transport, id) {
  ...
  if (transport === 'websocket') {
    // client.socket = io(url, { transports: ['websocket'], path: '/', rejectUnauthorized: false })
    client.socket = io('http://localhost:3030');
...
async function authenticateClient(client, strategy) {
  ...
  client.data.user = await client.service('/users').get(payload.userId)
