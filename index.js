...
program
    .usage('[options]')
    .option('-u, --url [url]', 'Base URL of the application to be tested', 'http://localhost:3030')
    .option('-t, --total [total]', 'Total # of virtual clients to be created', '200')
    .option('-c, --concurrency [concurrency]', '# of concurrent clients during the steady phase', '20')
    .option('-s, --scenarios [scenarios]', 'Total # of scenarios performed by each virtual client before disconnecting', '1')
    .option('-r, --ramp [ramp]', 'Duration in seconds of the ramp up/down phases', '60')
...
const logger = winston.createLogger({ level, transports: [ new winston.transports.Console({ colorize: true }) ] })
...
for (var i = 0; i < total; i++) {
  ...
  // For the first/last clients include the ramp option as well (Comment following lines to disable ramp-up / down)
